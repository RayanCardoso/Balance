...
| Status when a payment exists but no version is in effect | `Paid`, with a null expected amount | Recorded after validation flagged it as a spec-precision gap: the four status ACs cover paid-equal, paid-different and unpaid, but not money-out-with-no-expectation. `Divergent` would be wrong - there is no expectation to diverge from. Not reachable through the public API today, since registering a payment for such a month is rejected with `NO_VERSION_IN_EFFECT`; the branch is defensive and is now pinned by a test so a future operation cannot change its meaning unnoticed | n |
